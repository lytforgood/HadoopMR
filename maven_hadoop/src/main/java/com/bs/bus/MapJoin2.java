...
补天气
 */
public class MapJoin2 extends Configured implements Tool{
    public static class LeftOutJoinMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            String dd=array[1].substring(0, array[1].length()-2);
            String hh=array[1].substring(array[1].length()-2, array[1].length());
				//判断链接字段是否在map中存在
				city_secondPart = city_info.get(dd);
	            if(city_secondPart != null){
	            	outPutKey.set(array[0]+","+dd+","+hh+","+array[2]);
...
//				"hdfs://hadoop-master.dragon.org:9000/opt/data/wc/inputjion/tb_dim_city.dat"};
    	try {
            int returnCode =  ToolRunner.run(new MapJoin2(),args);
