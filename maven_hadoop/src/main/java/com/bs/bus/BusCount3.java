...
public class BusCount3 extends Configured implements Tool {
	...
	public static class BusMap extends
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			String[] array = value.toString().split(",");
				mykey.set(array[1] +","+ array[5].substring(0, array[5].length()-2)+","+array[5].substring(array[5].length()-2, array[5].length()));
//				mykey.set(bus.getLine_name() +","+ bus.getDeal_time()+","
//						+ bus.getTerminal_id());
				myval.set(one+","+array[7]+","+array[8]+","+array[9]);
	...
	public static class BusReduce extends
		...
		protected void reduce(Text key, Iterable<Text> values,
			...
			for (Text value : values) {
				...
				val.set(sum+","+array[1]+","+array[2]+","+array[3]);
			}
	...
	public static void main(String[] args) throws Exception {
		args = new String[] {
				"hdfs://hadoop-master.dragon.org:9000/bs/outputjion/",
				// "hdfs://hadoop-00:9000/home910/liyuting/output/" };
				"hdfs://hadoop-master.dragon.org:9000/bs/outputweather/" };
		// run mapreduce
		int status = ToolRunner.run(new BusCount3(), args);
		// 5 exit
		System.exit(status);
	}
}
